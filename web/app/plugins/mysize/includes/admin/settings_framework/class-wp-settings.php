...
if ( !class_exists( 'WooCommerce_Plugin_Boiler_Plate_WP_Settings' ) ) {
	class WooCommerce_Plugin_Boiler_Plate_WP_Settings {
        ...
        function do_settings_sections( $page ) {
        	...
        	echo "<script>window.addEventListener('message', (event) => {
        		if(['https://api-staging.mysz.io','https://api.mysz.io'].includes(event.origin) && event.data && event.data.type == 'TOKEN'){
        		if (jQuery('#wc_pbp_general_wc_pbp_retailer_token_textbox').val() != event.data.retailerToken) {
        			jQuery('#wc_pbp_general_wc_pbp_retailer_token_textbox').val(event.data.retailerToken);
        			jQuery('.wc_pbp_settings .submit input').click();
        		}
        		}
        	})</script>";

        	$domain = str_replace('http://', '', str_replace('https://', '', get_home_url()));
        	$url = $GLOBALS['url_api'];
        	echo '<iframe src="'. $url .'/woocommerce/success?domain=' . $domain . '" id="settingsIframe" frameborder="0"></iframe>';
